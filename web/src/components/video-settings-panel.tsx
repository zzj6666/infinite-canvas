...
import { boolConfig, isSeedanceFastModel, isSeedanceVideoConfig, normalizeSeedanceDuration, normalizeSeedanceRatio, normalizeSeedanceResolution, seedanceDurationOptions, seedanceRatioOptions, seedanceResolutionOptionsForModel } from "@/lib/seedance-video";
...
function SeedanceVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    ...
    const resolutionGridClass = resolutionOptions.length === 2 ? "grid-cols-2" : resolutionOptions.length === 3 ? "grid-cols-3" : "grid-cols-4";
            ...
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                {showTitle ? (
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <div className="text-base font-semibold tracking-tight">影像设置</div>
                ...
                <SettingGroup title="输出画质" color={theme.node.muted}>
                    <div className={`grid ${resolutionGridClass} gap-2`}>
                        {resolutionOptions.map((item) => (
                            <OptionPill key={item.value} selected={resolution === item.value} theme={theme} onClick={() => onConfigChange("vquality", item.value)}>
                                {item.label}
                            </OptionPill>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {seedanceRatioOptions.map((item) => (
                            <button
                                key={item.value}
                                type="button"
                                className={`h-10 cursor-pointer rounded-lg border px-2 text-sm font-medium transition hover:opacity-80 ${item.value === "adaptive" ? "col-span-2" : ""}`}
                                style={{ background: ratio === item.value ? theme.toolbar.activeBg : "transparent", borderColor: ratio === item.value ? theme.node.activeStroke : theme.node.stroke, color: theme.node.text }}
                                onMouseDown={(event) => event.stopPropagation()}
                                onClick={() => onConfigChange("size", item.value)}
                            >
                                {item.value === "adaptive" ? "自适应" : item.value}
                            </button>
                        ))}
                    </div>
                </SettingGroup>
                ...
                <SettingGroup title="输出选项" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke, background: theme.node.fill }}>
